...
def convert_dn_monthly(debt_per_compounding, compound_rate): #takes minimum loan payment per compounding and converts to minimum loan payment per month
    if compound_rate == "continuous":
...
def sum_of_monthly_payments(credit_lines): #takes dictionary and calculates sum of the recommended monthly and min payments
    ...
    for line in credit_lines:
        if "min_payment" in line:
            ...
            if line["derferment"]:
                 line["monthly_payment"] = loan_min_payment(line["balance"], line["term"], convert_dn_monthly(debt_per_compound(line["balance"], 0, line["compound_rate"]),line["compound_rate"]))
